	...
	class UrbRequest {
		...
		public function GetOpeningHours($from = '', $to = '') {
			...
			if(!preg_match($dateRegex, $from) || !preg_match($dateRegex, $to)) {
				throw new Exception('From and To parameters must be in YYYY-MM-DD format.');
			}
			...
			$this->Call('GET', 'openinghours?' . http_build_query(array('from' => $from, 'to' => $to)));
		...
		public function ValidatePostalCode($postalCode = '') {
			...
			// this regex only works for Swedish postcodes
			/*
		...
		public function ValidateDelivery($order = array()) {
			...
			if($this->httpStatus !== 200) {
				if($this->httpBody && isset($this->httpBody->developer_message)) {
					throw new Exception($this->httpBody->developer_message);
				}
				else {
					throw new Exception('HTTP ' . $this->httpStatus);
				}
		...
		protected function Call($method = 'GET', $url = '', $postData = array()) {
			...
			$json = ($method === 'POST' && $postData) ? json_encode($postData) : '';
